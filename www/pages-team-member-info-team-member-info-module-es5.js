...
var BarGraphHtmlComponent = /** @class */ (function () {
    function BarGraphHtmlComponent() {
        this.barChartData = [];
        this.colors = ['', '#c32329', '#f79221', '#fcd603', '#a2cd3a', '#539c44'];
    }
    BarGraphHtmlComponent.prototype.ngOnInit = function () { };
    BarGraphHtmlComponent.prototype.getBarStyle = function (barData) {
...
var TeamMemberInfoPage = /** @class */ (function () {
    ...
    TeamMemberInfoPage.prototype.segmentChanged = function ($event) {
        this.currentTab = $event.target.value;
        if (this.currentTab === 'Team') {
            this.getAllUserRatings();
        }
    };
    TeamMemberInfoPage.prototype.segmentChangedGraphs = function ($event) {
        this.currentTabGraph = $event.target.value;
        if (this.currentTab === 'Team') {
            this.getAllUserRatings();
        }
    };
    TeamMemberInfoPage.prototype.setButtonStatus = function () {
    ...
    TeamMemberInfoPage.prototype.getUserRatingsByDate = function (project, team) {
        var _this = this;
        this.appDataService.getUserRatingsByDate("user_project=" + project.id + "&user_team=" + team.id).subscribe(function (rating) {
            _this.lineGraphData = rating;
            _this.checkGraphResponse();
        });
    };
    TeamMemberInfoPage.prototype.checkGraphResponse = function () {
        var _this = this;
        this.showGraph = false;
        this.lineGraphData.forEach(function (d, k) {
            if (d.length > 0) {
                _this.showGraph = true;
            }
        });
    };
    TeamMemberInfoPage.prototype.getAllUserRatings = function () {
        var _this = this;
        this.appDataService.getAllUserRatings().subscribe(function (rating) {
            _this.lineGraphData = rating;
            console.log(_this.lineGraphData);
            _this.checkGraphResponse();
        });
        this.appDataService.getUserRatingComments().subscribe(function (comments) {
            _this.userComments = comments;
        });
        this.appDataService.getUserRatingDay().subscribe(function (dayRatings) {
            _this.userDayRatings = dayRatings;
        });
        this.project = '';
        this.sTeam = '';
    };
...
var PipesCountDownPipe = /** @class */ (function () {
    function PipesCountDownPipe() {
    }
    PipesCountDownPipe.prototype.transform = function (timeStamp) {
        var length = parseInt(timeStamp, 10).toString().length; // '0.89'
        var value = 0;
        var title = '';
        var time = '';
        if (length <= 2) {
            ...
            if (value < 8) {
                ...
                var weekNum = Math.floor(value / 7);
                time = weekNum;
            }
        }
        return time + " " + title;
